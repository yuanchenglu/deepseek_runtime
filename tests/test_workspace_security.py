...
class WorkspaceSecurityTests(unittest.TestCase):
    ...
    def test_internal_regular_file_is_read_and_searched(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(tools.read_file({"input": "src/main.py"}), "needle = 'inside'\n")
            self.assertIn("src/main.py:1:needle = 'inside'", tools.search({"input": "needle"}))
    ...
    def test_external_directory_symlink_is_never_traversed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            outside.mkdir()
            (outside / "secret.txt").write_text("directory-secret-marker", encoding="utf-8")
            link = workspace / "linked-dir"
            self._symlink(outside, link, directory=True)
            tools = WorkspaceTools(workspace)

            for _ in range(20):
                self.assertEqual(tools.search({"input": "directory-secret-marker"}), "No matches")
                with self.assertRaises(WorkspaceViolation):
    ...
    def test_git_directory_and_links_are_excluded_from_search(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(tools.search({"input": "hidden-marker"}), "No matches")
            self.assertIn("visible.txt", tools.search({"input": "visible-marker"}))
    ...
    def test_windows_reparse_attribute_is_classified_as_link(self) -> None:
        ...
        self.assertFalse(WorkspaceResolver.is_reparse_stat(SimpleNamespace(st_file_attributes=0)))  # type: ignore[arg-type]
    ...
    def test_windows_junction_is_not_traversed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            outside = base / "outside"
            outside.mkdir()
            (outside / "secret.txt").write_text("junction-secret-marker", encoding="utf-8")
            ...
            tools = WorkspaceTools(workspace)
            self.assertEqual(tools.search({"input": "junction-secret-marker"}), "No matches")
            with self.assertRaises(WorkspaceViolation):
