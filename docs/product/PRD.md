...
- “实现存在”不等于“验收通过”；只有可复现测试和发布证据才能标记 `Verified`。
...
1. 模块未形成端到端、不可绕过的执行闭环；
2. 安全和恢复语义存在越界、可绕过或不确定行为；
3. 产品承诺、测试证据和代码实现未统一。
...
每次工具执行都经过注册、参数校验、权限决策、必要审批、超时和输出预算。
...
首个 Alpha 不使用命令黑名单、cwd 限制或普通 subprocess 宣称“安全沙箱”。
...
| RUN-003 | P1 | 所有工具调用必须经 ToolRegistry | 无裸 handler 生产路径 | Blocked |
| RUN-004 | P1 | 所有关键状态转换可 checkpoint | transition/event/checkpoint 测试 | Planned |
| RUN-005 | P1 | max_steps 是明确预算 | 超限返回 `BUDGET_STEP_EXCEEDED` | Partial |
| RUN-006 | P1 | 支持 cancellation | provider/tool 执行可取消并清理 | Planned |
...
| RUN-010 | P1 | malformed Provider 不抛未处理异常 | fixtures 全部返回 RuntimeResult | Blocked |
...
| TOOL-005 | P1 | Tool 声明 timeout/output limit | 超时或超限结构化失败 | Planned |
...
| WS-002 | P0 | search 不能跟随外部 symlink/reparse point | adversarial tests | Blocked |
...
| SEC-005 | P1 | 命令 Gate 不宣称隔离 | 文档、类型和帮助文本一致 | Blocked |
| SEC-006 | P1 | 子进程使用最小环境变量 | child 无 API Key 测试 | Blocked |
| SEC-007 | P1 | 命令 timeout 清理进程树 | child-process test | Planned |
| SEC-008 | P1 | 提供可插拔 Execution/Sandbox Adapter 接口 | fake、no-isolation、restricted adapter tests | Planned |
...
| ID | P | 需求 | 验收标准 | 当前 |
| --- | --- | --- | --- | --- |
| CHG-001 | P0 | Rollback handle 不可伪造 | forged-handle test | Blocked |
| CHG-002 | P0 | rollback 路径重新执行 containment 和 policy | external-path test | Blocked |
...
| CHG-010 | P1 | rollback handle 引用受保护的 ChangeJournal；重启后语义明确 | restart/expired/cross-workspace tests | Planned |
...
| EVD-003 | P1 | evidence 对任意 JSON-compatible 输入是 total function | property test | Blocked |
...
| OSS-003 | P1 | Ruff + 单一 type checker | CI green | Planned |
| OSS-004 | P1 | Coverage 阈值和关键路径分母明确 | 指标定义与 gate 均通过 | Planned |
...
| OSS-009 | P1 | CONTRIBUTING/CODE_OF_CONDUCT/issue/PR templates 完整 | 文件和链接校验 | Planned |
| OSS-010 | P1 | 安全报告流程和响应范围明确 | SECURITY 内容测试和人工审查 | Partial |
| OSS-011 | P1 | README 所有公开能力有 PRD/测试证据 | traceability audit | Blocked |
...
- 所有工具参数必须在 handler 前验证。
- 默认日志不能包含正文和 secret。
- Runtime 不得把命令分类器、cwd 或普通 subprocess 描述为内核隔离。
